...
class TestSomCrmMailDomainBlacklist(TransactionCase):
    ...
    def setUp(self):
        ...
        self.initial_blacklist = _MAIL_DOMAIN_BLACKLIST.copy() if isinstance(_MAIL_DOMAIN_BLACKLIST, set) else set()
    ...
    def test_unlink_removes_from_blacklist(self):
        """Test that removing a domain from the blacklist model also removes it from the global _MAIL_DOMAIN_BLACKLIST."""
        from odoo.addons.iap.tools.iap_tools import _MAIL_DOMAIN_BLACKLIST
        ...
        self.assertIn(domain_name, _MAIL_DOMAIN_BLACKLIST, "The domain should be added to the blacklist upon creation.")
        
        record.unlink()
        self.assertNotIn(domain_name, _MAIL_DOMAIN_BLACKLIST, "The domain should be removed from the blacklist after unlink.")

    def test_unlink_keeps_duplicates(self):
        """Test that removing a domain doesn't remove it from the global _MAIL_DOMAIN_BLACKLIST if another record holds the same domain."""
        from odoo.addons.iap.tools.iap_tools import _MAIL_DOMAIN_BLACKLIST
        ...
        record = self.env['som.crm.mail.domain.blacklist'].search([('name', '=', domain_name)], limit=1)
        record.unlink()
        
        self.assertIn(domain_name, _MAIL_DOMAIN_BLACKLIST, "The domain should still be in the blacklist because another record has it.")
        
        remaining_records = self.env['som.crm.mail.domain.blacklist'].search([('name', '=', domain_name)])
        remaining_records.unlink()
        
        self.assertNotIn(domain_name, _MAIL_DOMAIN_BLACKLIST, "The domain should be removed from the blacklist once all duplicates are unlinked.")
